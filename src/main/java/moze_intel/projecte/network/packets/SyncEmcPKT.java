...
public class SyncEmcPKT implements IMessage
{
	...
	public void toBytes(ByteBuf buf)
	{
		...
		for (Object obj : data)
		{
			...
			buf.writeInt((Integer) array[0]);
			buf.writeInt((Integer) array[1]);
			buf.writeInt((Integer) array[2]);
			buf.writeLong((Long) array[3]);
		}
	}
	...
	public static class Handler implements IMessageHandler<SyncEmcPKT, IMessage>
	{
		...
		public IMessage onMessage(final SyncEmcPKT pkt, MessageContext ctx)
		{
			...
			for (Object obj : pkt.data)
			{
				...
				SimpleStack stack = new SimpleStack((Integer) array[0], (Integer) array[1], (Integer) array[2]);

				if (stack.isValid())
				{
					EMCMapper.emc.put(stack, (Long) array[3]);
				}
			}
